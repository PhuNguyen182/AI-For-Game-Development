...
|---|---|
| `research-decision.md` **E5** / **E6** | **E5**: the capability stated as behaviour, and whether a paid or closed-source option is acceptable — `researcher` returns `Blocked` on a topic rather than a behaviour. **E6**: the feasibility question **and the decision waiting on it** (`Blocked` without the second), the pass/fail threshold, the target hardware, and the current baseline. For an on-device measurement, also the GD's authorisation for `build-run-engineer` to produce the build — `rd-engineer` is barred from making one. Neither entry runs a step 0, so classify the run itself |
