...
| Ask for | Because |
|---|---|
| **Attempts used, out of the budget — and past the first, what the last attempt improved** | No implementing envelope carries either, and `assurance-evaluator` absent the count *assumes Attempt 1*, scoring an iterated submission as a first pass. In one live run of this pipeline, one agent volunteered the count and another did not, which is what an unrequired field looks like. The second half is owed for the same reason: `execution-loop.md` requires a retry to state what materially improved, the gate scores exactly that, and a live gate handed only `2 / 3` reported the iteration **unevidenced** rather than assuming it |
