...
| **Enforcement is advisory only** | Partly settled | This row said "reopen only when a miss supplies the evidence". A miss did: four self-describing counts had drifted and two cross-references pointed at blank lines, all found by audit rather than by any check. `tools/verify-workflow-layer.ps1` and `/verify-workflow-layer` close the **self-description** half at **95** machine-checked claims — 52 structural, then two semantic routing checks, one more per promoted reference, and in §15 two more: every agents table must declare `Produces` as its third column and fill it — the property the custody check silently depends on — and no consuming file may restate the gate ask as the orchestrator asking the GD. The semantic pair is the only part that has ever caught a *behavioural* defect, and it caught one in this round's own edits. A `PreToolUse` hook blocking a dispatch that skips the router stays **Deferred** — still no evidence of that miss |
...
| **`ci-cd-engineer` is reachable only in mode 3** | Recorded | Added at the GD's instruction with no Step 0 lane row, so sizing never routes to it and a CI/CD task still lands in the chore lane until the GD names the agent. Class counts (B is now 14, of which 12 accrue debt) and the `Routed to:` fallback are updated — the fallback shares `git-expert`'s row rather than adding one, because `orchestrator.md` sits at 199 lines against its 200 cap. The mermaid is not updated, because no lane changed. Closing it means adding a lane row — one round, only if auto-routing is wanted |
...
| **The custody class is written down, not machine-checked** | Settled for five of six — and §15 found the check itself could be **blind**: it reads the agents table's third column, and one pipeline's third column was not `Produces`, so it passed while six deliverables went unreceived. The column is now asserted too. `change-request.md` stays unasserted | Seventeen findings across two pipelines reduced to four classes, and the largest — *an agent produces something no pipeline row receives* — was closed **instance by instance**. `verify-workflow-layer.ps1` passed 78/78 with twelve of them live. Closing the class is fix **J**: (1) every artifact a pipeline declares under `Produces` must be named in at least one entry or hand-back row; (2) every `Routed to:` value an agent file instructs an agent to return must have a matching routing row in each pipeline that dispatches it. Both are negative-testable, both run across all six pipelines including the four unreviewed, and this is **the only route to a higher score that does not require a real project** |
