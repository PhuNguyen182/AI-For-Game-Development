...
```
For a Checkpoint 3 summary keep the same envelope and replace the body with `Built:`, `Matches spec intent:` (with any drift named), and `Known limitations:`. For a mid-flight GDD change, add one `Change severity:` line **per independently-severable part of the request** — Minor (update the spec in place), Moderate (roll back to Checkpoint 2), or Major (roll back to Checkpoint 1) — and list the code now needing rework. A part that forces a strategic technology choice gets no severity at all: return `Needs-decision`, `Routed to: cto` for that part, and still classify whatever the rest of the request can be rated on its own. At Major, write `Module boundaries:`, `Client-server contract:`, `Architecture diagram:`, `Patterns chosen:`, `Task breakdown:` and `Acceptance criteria:` as `Pending CP1` rather than inventing a direction the reopened loop has not yet settled — but still state what the change is meant to achieve, in the GD's own words, so `critic` has a goal to test against. **For a three-strikes root cause** keep the same envelope and replace the body with:
```
